...
async function upsertLeads(leads) {
  ...
  console.log(`[finder] Saved ${saved} new, skipped ${skipped} duplicates/invalid`);
  return saved;
}

async function run() {
  console.log('[finder] Starting — ' + new Date().toISOString());

  if (OUTREACH_ENABLED !== 'true') {
    console.log('[finder] OUTREACH_ENABLED is not "true" — exiting.');
    process.exit(0);
  }
  ...
  console.log(`[finder] Complete — found ${leads.length} total, saved ${totalSaved} new leads`);
}
